...
from detectron.roi_data.loader_wsl import RoIDataLoader
import detectron.modeling.wsl_heads as wsl_heads
import detectron.modeling.wsl_seg_heads as wsl_seg_heads
import detectron.modeling.optimizer_wsl as optim_wsl
import detectron.roi_data.minibatch_wsl as roi_data_minibatch
...
def generalized_wsl(model):
    return build_generic_wsl_detection_model(
        model,
        get_func(cfg.MODEL.CONV_BODY),
        add_roi_box_head_func=get_func(cfg.FAST_RCNN.ROI_BOX_HEAD),
        add_roi_mask_head_func=get_func(cfg.MRCNN.ROI_MASK_HEAD),
...
def gpu_0_generalized_wsl_cpg(model):
    return build_generic_wsl_detection_model(
        model,
        get_func(cfg.MODEL.CONV_BODY),
        add_roi_box_head_func=get_func(cfg.FAST_RCNN.ROI_BOX_HEAD),
...
def gpu_1_generalized_wsl_cpg(model):
    return build_generic_wsl_detection_model(
        model,
        get_func(cfg.MODEL.CONV_BODY),
        add_roi_box_head_func=get_func(cfg.FAST_RCNN.ROI_BOX_HEAD),
...
def gpu_2_generalized_wsl_cpg(model):
    return build_generic_wsl_detection_model(
        model,
        get_func(cfg.MODEL.CONV_BODY),
        add_roi_box_head_func=get_func(cfg.FAST_RCNN.ROI_BOX_HEAD),
...
def gpu_3_generalized_wsl_cpg(model):
    return build_generic_wsl_detection_model(
        model,
        get_func(cfg.MODEL.CONV_BODY),
        add_roi_box_head_func=get_func(cfg.FAST_RCNN.ROI_BOX_HEAD),
...
def build_generic_wsl_detection_model(model,
                                      add_conv_body_func,
                                      add_roi_box_head_func=None,
                                      add_roi_mask_head_func=None,
                                      add_roi_keypoint_head_func=None,
    ...
    def _single_gpu_build_func(model):
        ...
        if not cfg.MODEL.RPN_ONLY:
            # Add the Fast R-CNN head
            head_loss_gradients['box'] = _add_wsl_head(model,
        ...
        if cfg.MODEL.MASK_ON:
            # Add the mask head
            head_loss_gradients['mask'] = _add_wsl_roi_mask_head(
                model, add_roi_mask_head_func, blob_conv, dim_conv,
                spatial_scale_conv)
    ...
    optim_wsl.build_data_parallel_model(model, _single_gpu_build_func)
    return model
...
def _add_wsl_roi_mask_head(model, add_roi_mask_head_func, blob_in, dim_in,
                           spatial_scale_in):
    ...
    # Add the mask output
    blob_mask = wsl_seg_heads.add_seg_outputs(model, blob_mask_head,
                                              dim_mask_head)
    ...
    if not model.train:  # == inference
        ...
        loss_gradients = wsl_seg_heads.add_seg_losses(model, blob_mask)
    return loss_gradients
...
def fast_rcnn_frozen_features(model):
    ...
    return build_generic_detection_model(model,
                                         get_func(cfg.MODEL.CONV_BODY),
                                         add_roi_box_head_func=get_func(
