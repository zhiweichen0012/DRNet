...
def add_AlexNet_conv5_body(model):
    ...
    pool1 = model.MaxPool(norm1, 'pool1', kernel=3, pad=0, stride=2)
    ...
    pool2 = model.MaxPool(norm2, 'pool2', kernel=3, pad=0, stride=2)
...
def add_AlexNet_2fc_head(model, blob_in, dim_in, spatial_scale):
...
def add_AlexNet_roi_2fc_head(model, blob_in, dim_in, spatial_scale):
