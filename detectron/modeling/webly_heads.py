...
def add_cross_entropy_loss_self_weight(model, pred, label, loss, cpg=None):
    ...
    add_cross_entropy_loss(
        model, pred, label, loss, weight=pred + '_self_weight', cpg=cpg)
...
def add_webly_mining(model, prefix=''):
    add_cls_pred(prefix + 'rois_pred', prefix + 'cls_prob', model, prefix='')
...
def add_entropy_weight(model, rois_pred_blob, rois_blob):
    model.net.Split(
        rois_pred_blob, [rois_pred_blob + '_bg', rois_pred_blob + '_useless'],
        split=[1, model.num_classes - 2],
        axis=1)
    model.net.Concat(
    ...
    model.net.Split(
        rois_blob, [rois_blob + '_useless', rois_blob + '_4'],
        split=[1, 4],
        axis=1)
    model.net.Tile(
        rois_blob + '_4', rois_blob + '_fgbg', axis=1, tiles=model.num_classes)
    ...
    model.net.Max([rois_blob + '_entropy', 'labels_oh_inv'],
                  rois_blob + '_classes_weight')

    weight = rois_blob + '_classes_weight'
    return weight
...
def add_spatial_entropy_weight(model, rois_pred, cls_prob, rois):
    ...
    model.net.ReduceSum(
        rois_pred + '_hatE', rois_pred + '_hatE_sum', axes=[0], keepdims=True)
    ...
    # rois_pred + '__E_ave')
    # model.net.Scale(rois_pred + '__E_ave', rois_pred + '_E_ave', scale=-1.0)

    # model.net.MatMul([rois + '_J', rois_pred + '_E_ave'], rois_pred + '_D_ave')
    ...
    model.net.Scale([
        'labels_oh',
    ], ['labels_oh_x2'], scale=2.0)
    model.net.Sub(['labels_oh_one', 'labels_oh_x2'], ['labels_oh_one__x2'])
    model.net.Mul([rois_pred + '_hatE_sum_norm', 'labels_oh_one__x2'],
                  [rois_pred + '_hatE_sum_norm_sign'])

    l = model.net.Add([rois_pred + '_hatE_sum_norm_sign', 'labels_oh'],
                      [rois + '_classes_weight'])
    # --------------------------------------------------------------------
    ...
    # l = model.net.Add([cls_prob + '_selfrun', rois + '_classes_weight_'],
    # [rois + '_classes_weight'])
    ...
    model.net.Stat(
        rois + '_classes_weight',
        rois + '_classes_weight_stat',
        display=int(1280 / cfg.NUM_GPUS),
        # display=1,
        prefix='classes_weight')

    model.net.Stat(
        rois_pred + '_hatE_sum',
        rois_pred + '_hatE_sum_stat',
        display=int(1280 / cfg.NUM_GPUS),
        # display=1,
        prefix='hatE_sum')
...
def add_VGG16_roi_2fc_head_shared(model,
    ...
    l = model.FCShared(
        l, prefix + 'fc6', dim_in * 7 * 7, 4096, weight='fc6_w', bias='fc6_b')
    l = model.Relu(l, prefix + 'fc6')
    l = DropoutIfTraining(model, l, prefix + 'drop6', 0.5)
    l = model.FCShared(
        l, prefix + 'fc7', 4096, 4096, weight='fc7_w', bias='fc7_b')
    l = model.Relu(l, prefix + 'fc7')
    l = DropoutIfTraining(model, l, prefix + 'drop7', 0.5)
