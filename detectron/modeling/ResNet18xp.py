...
def add_shallow_ResNet_convX_body(model, block_counts):
    ...
    dim_in_old = dim_in
    s, dim_in = add_stage(
        model, 'res4d2', s, n3, dim_in, 256, 0, 2
    )
    n4 = block_counts[3]
    s, dim_in = add_stage(
        model, 'res5d4', s, n4, dim_in, 512, 0, 4
    )
    pyramids[0] += [s]
    pyramids[1] += [dim_in]
    pyramids[2] += [1. / 4]
    ...
    if len(block_counts) == 4:
        ...
        dim_in_old = dim_in
        s, dim_in = add_stage(
            model, 'res5d2', s, n4, dim_in, 512, 0,
        ...
        s, dim_in = add_stage(
            model, 'res5', s, n4, dim_in, 512, 0, 1
        )
        pyramids[0] += [s]
        pyramids[1] += [dim_in]
        pyramids[2] += [1. / 16]
        ...
        return pyramids[0], pyramids[1], pyramids[2]
    else:
        # return s, dim_in, 1. / 8.
        return pyramids[0], pyramids[1], pyramids[2]
