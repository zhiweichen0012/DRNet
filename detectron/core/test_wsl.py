...
def im_detect_all(model, im, box_proposals, obn_scores, timers=None):
    ...
    return cls_boxes, cls_segms, cls_keyps, masks
...
def im_detect_bbox(model,
    ...
    """
    boxes_o = boxes
    inputs, im_scale = _get_blobs(im, boxes, obn_scores, target_scale,
                                  target_max_size)
    ...
    # print('bbox: ', scores.max(), scores.min(), scores.mean())
    ...
    if cfg.TEST.BBOX_REG:
        ...
        pred_boxes = np.tile(boxes_o, (1, scores.shape[1]))
...
def im_detect_bbox_aug(model, im, box_proposals=None, obn_scores=None):
    ...
    def add_preds_t(scores_t, boxes_t):
        ...
        if cfg.TEST.BBOX_AUG.COORD_HEUR == 'ID':
            assert np.array_equal(
                boxes_ts[0], boxes_t), 'boxes at each scale should be the same'
    ...
    for aspect_ratio in cfg.TEST.BBOX_AUG.ASPECT_RATIOS:
        scores_ar, boxes_ar = im_detect_bbox_aspect_ratio(
            model, im, aspect_ratio, box_proposals, obn_scores)
        ...
        if cfg.TEST.BBOX_AUG.ASPECT_RATIO_H_FLIP:
            scores_ar_hf, boxes_ar_hf = im_detect_bbox_aspect_ratio(
                model, im, aspect_ratio, box_proposals, obn_scores, hflip=True)
    ...
    elif cfg.TEST.BBOX_AUG.SCORE_HEUR == 'SUM':
        scores_c = np.sum(scores_ts, axis=0)
...
def im_detect_mask_deeplab(model, im_scale, boxes):
    ...
    # core.ScopedName('mask_fc8')
    # ).squeeze()
    # print('mask_fc8: ', pred_masks.max(), pred_masks.min(), pred_masks.mean())

    pred_masks = workspace.FetchBlob(core.ScopedName('mask_fc8_bgfg'))
    # .squeeze()
    print('mask bgfg: ', pred_masks.shape, pred_masks.max(), pred_masks.min(),
          pred_masks.mean())

    # pred_masks = workspace.FetchBlob(
    # core.ScopedName('mask_fc8_probs')
    # ).squeeze()
    # print('mask_fc8_probs: ', pred_masks.max(), pred_masks.min(), pred_masks.mean())

    pred_masks_crf = workspace.FetchBlob(core.ScopedName('mask_fc8_crf'))
    # .squeeze()
    print('mask_fc8_crf: ', pred_masks_crf.shape, pred_masks_crf.max(),
          pred_masks_crf.min(), pred_masks_crf.mean())
...
def im_detect_mask(model, im_scale, boxes):
    ...
    if 'deeplab' in cfg.MRCNN.ROI_MASK_HEAD:
        return im_detect_mask_deeplab(model, im_scale, boxes)
...
def im_detect_mask_aug(model, im, boxes):
    ...
    if 'deeplab' in cfg.MRCNN.ROI_MASK_HEAD:
        _, _, h, w = masks_ts[0].shape
        masks_ts_new = []
        for m in masks_ts:
            m_new = []
            for c in range(cfg.MODEL.NUM_CLASSES):
                m_c_new = cv2.resize(m[:, c, :, :].squeeze(), (w, h))
                m_new.append(m_c_new)
            m_new = np.array(m_new, dtype=np.float32)
            m_new = m_new[np.newaxis, :, :, :]
            masks_ts_new.append(m_new)
        masks_ts = masks_ts_new
...
def box_results_with_nms_and_limit(scores, boxes):
    ...
    for j in range(1, num_classes):
        ...
        dets_j = np.hstack(
            (boxes_j, scores_j[:, np.newaxis])).astype(np.float32, copy=False)
...
def segm_results_deeplab(cls_boxes, masks, ref_boxes, im_h, im_w):
    num_classes = cfg.MODEL.NUM_CLASSES
    ...
    for j in range(1, num_classes):
        ...
        for _ in range(cls_boxes[j].shape[0]):
            ...
            im_mask[y_0:y_1, x_0:x_1] = mask[y_0:y_1, x_0:x_1]
...
def segm_results(cls_boxes, masks, ref_boxes, im_h, im_w):
    ...
    for j in range(1, num_classes):
        ...
        for _ in range(cls_boxes[j].shape[0]):
            ...
            im_mask[y_0:y_1, x_0:x_1] = mask[(y_0 - ref_box[1]):(
                y_1 - ref_box[1]), (x_0 - ref_box[0]):(x_1 - ref_box[0])]
