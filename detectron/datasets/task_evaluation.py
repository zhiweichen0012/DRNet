...
def evaluate_boxes(dataset, all_boxes, output_dir, use_matlab=False):
    ...
    if _use_json_dataset_evaluator(dataset):
        ...
        _ = _coco_eval_to_box_results(coco_eval)
    else:
...
def evaluate_masks(dataset, all_boxes, all_segms, output_dir):
    ...
    not_comp = not cfg.TEST.COMPETITION_MODE
    if _use_json_dataset_evaluator(dataset):
        coco_eval = json_dataset_evaluator.evaluate_masks(dataset,
                                                          all_boxes,
                                                          all_segms,
                                                          output_dir,
                                                          use_salt=not_comp,
        ...
        cs_eval = cs_json_dataset_evaluator.evaluate_masks(dataset,
                                                           all_boxes,
                                                           all_segms,
                                                           output_dir,
                                                           use_salt=not_comp,
    ...
    for dataset, task, metric, expected_val in cfg.EXPECTED_RESULTS:
        ...
        if isinstance(expected_val, list):
            ...
            msg = ('{} > {} > {} sanity check (actual vs. expected): '
                   ...
                   ).format(dataset, task, metric, actual_val, mean, std, lo,
                            hi)
        else:
            ...
            msg = ('{} > {} > {} sanity check (actual vs. expected): '
                   '{:.3f} vs. {:.3f}, err={:.3f}, tol={:.3f}').format(
                       dataset, task, metric, actual_val, expected_val, err,
                       tol)
        if not ok:
            msg = 'FAIL: ' + msg
            logger.error(msg)
            if cfg.EXPECTED_RESULTS_EMAIL != '':
                ...
                job_name = os.environ[
                    'DETECTRON_JOB_NAME'] if 'DETECTRON_JOB_NAME' in os.environ else '<unknown>'
                job_id = os.environ[
                    'WORKFLOW_RUN_ID'] if 'WORKFLOW_RUN_ID' in os.environ else '<unknown>'
                body = [
                    'Name:',
                    job_name,
                    'Run ID:',
